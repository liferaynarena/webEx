...
<%@page import="com.liferay.portlet.dynamicdatamapping.service.persistence.DDMStructureFinderUtil"%>
	...
	<aui:fieldset>
		...
		<div class="display-template">
			...
			<liferay-ui:ddm-template-selector
				...
				displayStyle="<%= assetDisplayStyle.length==2?assetDisplayStyle[0]:displayStyle%>"
	   ...
	   <div class="display-template">
			...
			<%
			TemplateHandler assetTemplateHandler = TemplateHandlerRegistryUtil.getTemplateHandler(AssetEntry.class.getName());
			%>

			<liferay-ui:ddm-template-selector
				classNameId="<%= PortalUtil.getClassNameId(assetTemplateHandler.getClassName()) %>"
				displayStyle="<%=assetDisplayStyle.length==2?assetDisplayStyle[1]:displayStyle%>"
				displayStyleGroupId="<%= displayStyleGroupId %>"
				refreshURL="<%= currentURL %>"
				showEmptyOption="<%= true %>"
			/>
		...
		<aui:select label="Structure" name="preferences--allAssetStructure--">
		<%
		...
		%>
	       <aui:option label="<%=ddmStructures.get(i).getName(locale)%>"  value="<%=ddmStructures.get(i).getPrimaryKey()%>" />
		<%} %>
		</aui:select>
